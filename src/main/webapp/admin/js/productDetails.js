...
$(document).ready(function () {
 ...
 /* $("#listAdd").on("click",function(){
	  //新建批量上传的页面
	  var str='<div class="div-detail"><span class="label-tip">商品种类</span><input type="text" name="product[0].kind[0]" /></div>';
	  str+='<div class="div-detail"><span class="label-tip">商品数量</span> <input type="text" name="product[0].number[0]" /></div>';
  ...
  $("#kindcheck").on('click',function(){
	  var colornumber=$("#colornumber").val();
	  var connnumber=$("#connnumber").val();
	  var romnumber=$("#romnumber").val();
	  var productnumbers=colornumber*connnumber*romnumber; 
	  var i =0;
	  $("#kindsdiv").remove();
	  var str='<div class="div-detail"><span class="label-tip col-md-2">商品颜色</span>'+
	  '<span class="label-tip col-md-2">入网方式</span><span class="label-tip col-md-2">内存</span>'+
	  '<span class="label-tip col-md-2">数量</span><input type="text" style="visibility: hidden;"/></div>';//头
	  for(i=0;i<productnumbers;i++){
		  str+='<div class="div-deatil">';
		  str+="<input type='text' name='product.color["+i+"]'/>";//颜色
		  str+="<input type='text' name='product.conn["+i+"]'/>";//入网方式
		  str+="<input type='text' name='product.rom["+i+"]'/>";//内存
		  str+="<input type='text' name='product.number["+i+"]'/>";//商品数量
		  str+="</div>";
	  }
	  $("#container").append(str);
